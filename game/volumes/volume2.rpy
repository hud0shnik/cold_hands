...
label v2_s1:
    ...
    scene bg void
    with fade
    "Утро субботы. Я просыпаюсь от того, что меня будит Яна."
    ...
    "Так как я стараюсь в последнее время чаще покидать квартиру, я хожу за покупками вместе с Яной."
    with fade
    stop music
...
label v2_s2_yana:
    ...
    play sound door
    with fade
    show y 1-5 at center with dissolve
    D "Как настроение?"
...
label v2_s2_oolong:
    ...
    "Ну, улун допил, пора направляться в лучшую комнату квартиры."
...
label v2_s3:
    ...
    "Яна уже ушла на собеседование.... Делать мне нечего."
...
label v2_s4:
    ...
    play sound door
    show y 1-1 at center with dissolve
    y "Я вернулась."
    D "Как прошло?"
    y 1-4 "Ну, как тебе сказать. Персонал весьма дружный, владелица сразу взяла под крыло."
    y "Со следующего понедельника выхожу, буду пять дней работать с текущим менеджером, потом сама."
    ...
    if coldPoints > 2:
        D "Все вопросы к нему. Ладно, пойду полежу."
        y 1-2 "Ага."
        "Самое время послушать какой-нибудь детективчик."
    else:
        D "Все вопросы к нему. Так вот, не думаю, что вернусь под час ночи, так что не переживай."
...
label v2_s5:
    ...
    "У меня на плеере было около трёх детективов, но один мне уже не зашёл. Варианта два."
...
label v2_s5_camp:
    ...
    "Главный герой - милиционер  старой закалки. После недели поисков он находит в лесу останки жертв."
...
label v2_s6:
    ...
    "Скорее всего уже наступил вечер. Яна что-то делает на кухне, пойду поинтересуюсь."
    play sound door
    with fade
    show y 1-1 at center with dissolve
    ...
    if mindPoints >= 2:
        ...
        D "Ага..... Ну, ладно, зато на своих ошибках учиться легче."
        y "Да уж."
